...
The **bioacoustics** package contains all the necessary R functions to read audio recordings of various formats (*e.g.*, WAV, WAC, MP3, Zero-Crossing), filter noisy files, display audio signals, detect and extract automatically acoustic features for further analysis such as species identification based on classification of animal vocalizations. This package does not provide R functions to perform classification tasks. Other packages such as **randomForest**, **extraTrees**, **mclust**, or **keras** can be used in addition with the **bioacoustics** package to perform these tasks. Complementary functions for audio processing in R are also available in the **tuneR**, **monitorR**, **warbleR**, and **seewave** packages.

This package was originally built for bat bioacoustics, and thus the default arguments for all the available functions are set for bats. Users will have to set the arguments according to the group of animal vocalizations they which to study^[See Table 1 for examples of extraction arguments for different species groups]. The **bioacoustics** package can be used to display and extract acoustic features from recordings of birds, frogs, insects, marine and other terrestrial mammals as long as their vocalizations can be recorded.

This package is subdivided into three main components containing R functions accessible to users and internal processes that will also be defined below for the sake of clarity and ease of comprehension. The three main components with the current functions are as follow:


- Read, write, display, extract data (not yet implemented) from Zero-Crossing files: `read_zc`, `plot_zc`, `metadata_zc`, `write_zc`

- Stand-alone tools to display, convert or resample audio recordings: `read_audio`, `metadata`, `resample`, `mp3_to_wav`, `spectro` 

- Extract acoustic features from audio recordings: `fspec`, `threshold_detection`, `blob_detection`
...
The `read_audio` function loads into R mono and stereo audio recordings with MP3, Wave, or proprietary compressed WAC (from Wildlife Acoustics) formats. It can be used as a stand-alone function and is used as an helper in all the functions dealing with audio recordings. <!-- The `read_wac` function was built using the  necesary information from the wac2wavcmd-1.1 (provided by Wildlife Acoustics under the MIT open software license) to decode a WAC file.-->


## Extract metadata

The `metadata` function automatically extracts metadata encapsulated in audio recordings that were previously loaded into R with the `read_audio` function. It can also extract metadata from objects generated with the `threshold_detection` or `blob_detection` functions. GUANO metadata extraction is not yet implemented with this function. <!--is a modified version of the wa_meta-1.0 Wildlife Acoustics metadata reader. The metadata encapsulated in a Wave file produced by some of their bioacoustics recorders, including (with the latest firmware) the SM2, SM2+, SM2BAT192x2, SM2BAT384, SM2BAT+, SM3, SM3BAT, EM3, EM3+ and Echo Meter Touch, can be read and extracted with this function.--> 
...
The extracted audio event sequence is smoothed with a Kalman filtering function, whose parameters can also be set using the `KPE` (Kalman Process Error) and `KME` (Kalman Measurement Error) arguments. Another series of filtering thresholds are applied to the Kalman filtered outputs such as minimum duration (`min_dur`), maximum duration (`max_dur`), minimum time between two audio events (`TBE`). Acoustic features are then extracted from the filtered frequency (Hz) and energy (dB) outputs.
...
The metadata embeded in the Wave file header can be extracted by setting the `metadata` argument to `TRUE`. Note that the metadata can also be extracted independently from the `threshold_detection` function by using the `metadata` function

The detection and extraction settings used with the `threshold_detection` and `blob_detection` functions can be saved along with the metadata by setting the `settings` argument to `TRUE`.
...
A series of filtering functions are also applied to the segments such as minimum duration (`min_dur`), maximum duration (`max_dur`), minimum time between two segments (`TBE`) to reduce the amount of unwanted noise or to track particuliar segments within a narrower temporal window. Like temporal filters, High Pass (HPF) and Low Pass frequency filters (LPF) can also be set using the `HPF` and `LPF` arguments in the `blob_detection` function. Frequencies below the HPF and above the LPF cutoff are greatly attenuated. Acoustic features are then extracted from these filtered segments.
...
### Myotis
The myotis dataset is a Wave file of 10 seconds, 16 bits, mono, with an original sampling rate at 500 kHz (time expanded by 10). It contains 11 echolocation calls of bats from the *Myotis* genus. The recording was made in United-Kingdom with a D500X bat detector from Pettersson Elektronik AB.
